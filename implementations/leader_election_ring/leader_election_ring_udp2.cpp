#include "leader_election_ring_udp2.h"
...
#include <cstdint>
#endif
typedef leader_election_ring_udp2 ivy_class;
...
void leader_election_ring_udp2::install_reader(reader *r) {
    #ifdef _WIN32
...
void leader_election_ring_udp2::install_thread(reader *r) {
    install_reader(r);
}

void leader_election_ring_udp2::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void leader_election_ring_udp2::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void leader_election_ring_udp2::__unlock() { ReleaseMutex(mutex); }
#else
    void leader_election_ring_udp2::__lock() { pthread_mutex_lock(&mutex); }
    void leader_election_ring_udp2::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
struct thunk__sec__impl__handle_timeout{
    leader_election_ring_udp2 *__ivy;
    thunk__sec__impl__handle_timeout(leader_election_ring_udp2 *__ivy): __ivy(__ivy){}
    void operator()() const {
...
struct thunk__trans__impl__handle_recv{
    leader_election_ring_udp2 *__ivy;
    thunk__trans__impl__handle_recv(leader_election_ring_udp2 *__ivy): __ivy(__ivy){}
...
//                    std::cout << "binding id: " << my_id << " port: " << ntohs(myaddr.sin_port) << std::endl;
    ...
    int leader_election_ring_udp2::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
void leader_election_ring_udp2::imp__serv__elect(){
    {
    }
}
void leader_election_ring_udp2::ext__trans__recv(unsigned v){
    {
        if((v == asgn__pid[me])){
            ext__serv__elect();
        }
            ...
            if((asgn__pid[me] < v)){
                {
                    unsigned loc__0;
    loc__0 = (unsigned)___ivy_choose(0,"loc:0",30);
                    {
...
void leader_election_ring_udp2::ext__sec__timeout(){
    {
        {
            unsigned loc__0;
    loc__0 = (unsigned)___ivy_choose(0,"loc:0",31);
            {
                loc__0 = ext__node__get_next(me);
...
void leader_election_ring_udp2::__init(){
    {
    }
}
void leader_election_ring_udp2::trans__impl__handle_recv(unsigned x){
    ext__trans__recv(x);
}
void leader_election_ring_udp2::sec__impl__handle_timeout(){
    ext__sec__timeout();
}
void leader_election_ring_udp2::ext__trans__send(unsigned dst, unsigned v){
    {
        trans__impl__rdr->write(dst,v);
    }
}
void leader_election_ring_udp2::ext__serv__elect(){
    imp__serv__elect();
}
unsigned leader_election_ring_udp2::ext__node__get_next(unsigned x){
    unsigned y;
...
void leader_election_ring_udp2::__tick(int __timeout){
}
leader_election_ring_udp2::leader_election_ring_udp2(unsigned me){
#ifdef _WIN32
...
leader_election_ring_udp2::~leader_election_ring_udp2(){
...
int ask_ret(long long bound) {
    ...
    class leader_election_ring_udp2_repl : public leader_election_ring_udp2 {
    ...
    leader_election_ring_udp2_repl(unsigned me) : leader_election_ring_udp2(me){}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    leader_election_ring_udp2_repl &ivy;    

    cmd_reader(leader_election_ring_udp2_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
...
int main(int argc, char **argv){
    ...
    if (argc != 2){
        std::cerr << "usage: leader_election_ring_udp2 me\n";
        __ivy_exit(1);
    }
...
#endif
    leader_election_ring_udp2_repl ivy(p__me);
