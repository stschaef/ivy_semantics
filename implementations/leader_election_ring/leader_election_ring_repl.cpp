#include "leader_election_ring_repl.h"
...
#include <cstdint>
#endif
typedef leader_election_ring_repl ivy_class;
...
void leader_election_ring_repl::install_reader(reader *r) {
    #ifdef _WIN32
...
void leader_election_ring_repl::install_thread(reader *r) {
    install_reader(r);
}

void leader_election_ring_repl::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void leader_election_ring_repl::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void leader_election_ring_repl::__unlock() { ReleaseMutex(mutex); }
#else
    void leader_election_ring_repl::__lock() { pthread_mutex_lock(&mutex); }
    void leader_election_ring_repl::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
int leader_election_ring_repl::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
void leader_election_ring_repl::imp__serv__elect(unsigned v){
    {
    }
}
void leader_election_ring_repl::ext__app__async(unsigned me){
    {
        unsigned loc__0;
    loc__0 = (unsigned)___ivy_choose(0,"loc:0",27);
        {
...
void leader_election_ring_repl::ext__trans__recv(unsigned dst, unsigned v){
    {
        if((v == asgn__pid[dst])){
            ext__serv__elect(dst);
        }
        else {
            if((asgn__pid[dst] < v)){
                {
                    unsigned loc__0;
    loc__0 = (unsigned)___ivy_choose(0,"loc:0",28);
                    {
                        loc__0 = ext__node__get_next(dst);
...
void leader_election_ring_repl::ext__serv__elect(unsigned v){
    imp__serv__elect(v);
}
void leader_election_ring_repl::imp__trans__send(unsigned dst, unsigned v){
    {
    }
}
void leader_election_ring_repl::ext__trans__send(unsigned dst, unsigned v){
    imp__trans__send(dst, v);
}
unsigned leader_election_ring_repl::ext__node__get_next(unsigned x){
    unsigned y;
...
void leader_election_ring_repl::__init(){
    {
    }
}
void leader_election_ring_repl::__tick(int __timeout){
}
leader_election_ring_repl::leader_election_ring_repl(){
#ifdef _WIN32
...
int ask_ret(long long bound) {
    ...
    class leader_election_ring_repl_repl : public leader_election_ring_repl {
    ...
    leader_election_ring_repl_repl() : leader_election_ring_repl(){}
    virtual void imp__serv__elect(unsigned v){
    __ivy_out  << "< serv.elect" << "(" << v << ")" << std::endl;
}
    virtual void imp__trans__send(unsigned dst, unsigned v){
    __ivy_out  << "< trans.send" << "(" << dst << "," << v << ")" << std::endl;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    leader_election_ring_repl_repl &ivy;    

    cmd_reader(leader_election_ring_repl_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "app.async") {
                    ...
                    ivy.ext__app__async(_arg<unsigned>(args,0,2));
                }
                else
    
                if (action == "trans.recv") {
                    check_arity(args,2,action);
                    ivy.ext__trans__recv(_arg<unsigned>(args,0,2),_arg<unsigned>(args,1,2));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: leader_election_ring_repl \n";
        __ivy_exit(1);
    }
