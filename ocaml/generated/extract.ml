...
let nondet_helper t type_sizes =
  ...
  | Ivytype_User_Defined _ ->
    Some (Expr_EnumVarLiteral (t,
      (Z.to_nat
        (hd
          (rand Z0 (Zpos (XO XH)) (Z.add (Z.of_nat (type_sizes t)) (Zpos XH)))))))
  | _ -> None
             ...
             (match small_step_Expr e0 var_ctx fun_var_ctx act_ctx
              ...
              | None ->
                (match es' with
                 | [] -> None
                 | e' :: es'' ->
                   (match small_step_Expr e' var_ctx fun_var_ctx act_ctx
                            declared_types type_sizes with
                    | Some e'' ->
  ...
  | Expr_Not e' ->
    ...
    then (match e' with
          ...
          | _ -> None)
           ...
           small_step_Expr e' var_ctx fun_var_ctx act_ctx declared_types
             type_sizes
         in
  ...
  | Expr_And (e1, e2) ->
    ...
    then if is_value e2
         then (match e1 with
           ...
           ((fromMaybe Expr_Error
              (small_step_Expr e1 var_ctx fun_var_ctx act_ctx declared_types
                type_sizes)), e2))
  | Expr_Or (e1, e2) ->
    if is_value e1
    then if is_value e2
