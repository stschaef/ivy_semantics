...
let rec eqb_Ivytype t1 t2 =
  ...
  | Ivytype_Bool -> (match t2 with
                     | Ivytype_Bool -> true
                     | _ -> false)
  | Ivytype_User_Defined s1 -> (match t2 with
                                | Ivytype_User_Defined s2 -> eqb0 s1 s2
                                | _ -> false)
  | Ivytype_Fun (ts1, t3) ->
    (match t2 with
     | Ivytype_Fun (ts2, t4) -> (&&) (list_beq eqb_Ivytype ts1 ts2) (eqb_Ivytype t3 t4)
     | _ -> false)
...
let rec eqb_Expr e1 e2 =
  ...
  | Expr_EnumVarLiteral (t1, n1) ->
    (match e2 with
     | Expr_EnumVarLiteral (t2, n2) -> (&&) (eqb_Ivytype t1 t2) (eqb n1 n2)
     | _ -> false)
  | Expr_VarFun (x1, es1) ->
    (match e2 with
  ...
  | Expr_And (e11, e12) ->
    (match e2 with
     ...
     | _ -> false)
  | Expr_Or (e11, e12) -> (match e2 with
                           | Expr_Or (e21, e22) -> (&&) (eqb_Expr e11 e21) (eqb_Expr e12 e22)
                           | _ -> false)
  | Expr_Eq (e11, e12) -> (match e2 with
...
let rec rand seed n1 n2 =
  let seed' = Z.modulo seed n2 in lazy (Cons (seed', (rand (Z.mul seed' n1) n1 n2)))

(** val nondet_helper : ivytype -> enumTypeSizes -> expr option **)

let nondet_helper t type_sizes =
  match t with
  | Ivytype_Bool -> if Z.eqb (hd (rand Z0 (Zpos (XO XH)) (Zpos (XI XH)))) Z0 then Some Expr_True else Some Expr_False
  | Ivytype_User_Defined _ ->
    Some (Expr_EnumVarLiteral (t,
      (Z.to_nat (hd (rand Z0 (Zpos (XO XH)) (Z.add (Z.of_nat (type_sizes t)) (Zpos XH)))))))
  | _ -> None

(** val small_step_Expr :
    expr -> context -> context -> action_context -> (ivytype -> bool) -> enumTypeSizes -> expr option **)
...
let rec small_step_Expr e var_ctx fun_var_ctx act_ctx declared_types type_sizes =
       ...
       (match i with
        | Ivytype_Fun (_, _) ->
          (match es with
           | [] -> None
           | e0 :: es' ->
             (match small_step_Expr e0 var_ctx fun_var_ctx act_ctx declared_types type_sizes with
                ...
                (match es' with
                 ...
                 | e' :: es'' ->
                   (match small_step_Expr e' var_ctx fun_var_ctx act_ctx declared_types type_sizes with
                    | Some e'' -> Some (Expr_VarFun (x, (e0 :: (e'' :: es''))))
                    | None -> None))))
        | _ -> None)
     | None -> None)
  | Expr_Not e' ->
    ...
    then (match e' with
          ...
          | _ -> None)
    else let e'' = small_step_Expr e' var_ctx fun_var_ctx act_ctx declared_types type_sizes in
    ...
    then if is_value e2
         then (match e1 with
               ...
               | _ -> None)
                ...
                (fromMaybe Expr_Error (small_step_Expr e2 var_ctx fun_var_ctx act_ctx declared_types type_sizes))))
    else Some (Expr_And
           ((fromMaybe Expr_Error (small_step_Expr e1 var_ctx fun_var_ctx act_ctx declared_types type_sizes)), e2))
  | Expr_Or (e1, e2) ->
    if is_value e1
    then if is_value e2
         then (match e1 with
  ...
  | Expr_Exists (x, t, e') ->
    let out =
      fold_left (fun acc y -> Expr_Or (acc, (subst e' (Expr_EnumVarLiteral (t, y)) x))) (seq O (type_sizes t))
        Expr_False
    in
    Some out
  ...
  | Expr_Cond (e1, e2, e3) ->
    ...
    then (match e1 with
          ...
          | _ -> None)
           ...
           ((fromMaybe Expr_Error (small_step_Expr e1 var_ctx fun_var_ctx act_ctx declared_types type_sizes)), e2, e3))
  | _ -> None
