...
let update_action_context c x params ret_type body =
  ...
  build_context vc tc ((fun y ->
    ...
    else fst ac y), (x :: (snd ac)))

(** val empty_context : context **)

let empty_context =
  build_context ((fun _ -> None), [])
...
let rec check_command_helper p gamma =
  ...
  | Com_TypeDecl (t_id, n) ->
    (match lookup_type gamma t_id with
     ...
     | None ->
       Some
         (update_type_context gamma t_id n))
...
let rec small_step_Expr e gamma s =
  ...
  | Expr_Cond (e1, e2, e3) ->
    ...
    then (match small_step_Expr e2 gamma s with
         ...
         then (match small_step_Expr e3
                       gamma s with
               | Some e'' ->
                 Some (Expr_Cond (e1, e2,
                   e''))
               | None -> None)
         else if is_value e1
              then (match e1 with
...
let rec small_step_Com p gamma s =
  ...
  | Com_Assign (x, e) ->
    if is_value e
    then Some (Com_Skip,
           (update_state s (Expr_VarLiteral
             x) e))
    else (match small_step_Expr e gamma s with
          | Some e' ->
