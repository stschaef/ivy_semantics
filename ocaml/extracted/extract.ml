...
| Expr_Forall of char list
   * ivytype * expr
| Expr_Exists of char list
   * ivytype * expr
| Expr_Cond of expr * expr
   * expr
| Expr_Error
...
| Com_GlobalFuncVarDecl of 
   ...
   * ivytype
| Com_TypeDecl of char list
   * nat
| Com_ActionDecl of char list
   * (char list * ivytype) list
   * ivytype * com
| Com_Interpret of ivytype
   * char list
| Com_Skip
...
let update_variable_context c x t =
  let vc =
    get_variable_context c
  in
  let tc = get_type_context c in
  let ac = get_action_context c
  in
  let fc =
    get_function_variable_context
      c
  in
  build_context ((fun y ->
    if eqb0 x y
...
let rec subst e v x =
  ...
  | Expr_FunctionSymbol (
      z, es) ->
    Expr_FunctionSymbol (z,
      (map (fun e' ->
        subst e' v x) es))
  | Expr_Not e' ->
    Expr_Not (subst e' v x)
  | Expr_And (e1, e2) ->
...
(** val type_expr :
    expr -> context -> ivytype
    option **)

let rec type_expr e gamma =
  match e with
  | Expr_VarLiteral x ->
    lookup_variable gamma x
  | Expr_EnumVarLiteral (
      t, n) ->
    (match t with
     | Ivytype_UserDefined (
         x, l) ->
       (match lookup_type gamma
                x with
        | Some m ->
          if (&&) (Nat.ltb m n)
               (Nat.eqb l m)
...
let rec check_command_helper p gamma =
  ...
  | Com_GlobalFuncVarDecl (
      var_id, arg_names_and_ts,
      ret_type) ->
    if eqb_OptionIvytype
         (lookup_variable gamma
           var_id) None
...
let rec small_step_Expr_list es gamma s =
  ...
  | e :: es' ->
    if is_value e
    then (match small_step_Expr_list
                  es' gamma s with
          | Some e' ->
            Some (e :: e')
          | None -> None)
...
let rec small_step_Com p gamma s =
  ...
  | Com_Call (x, args) ->
    let args_are_values =
      forallb is_value args
    in
    if args_are_values
    then (match lookup_action
                  gamma x with
          | Some p0 ->
            let (p1, p') = p0 in
            let (arg_ids_and_ts,
