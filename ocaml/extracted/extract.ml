...
let rec eqb_Expr e1 e2 =
  ...
  | Expr_EnumVarLiteral (t1, n1) ->
    (match e2 with
     | Expr_EnumVarLiteral (t2, n2) -> (&&) (eqb_Ivytype t1 t2) (Nat.eqb n1 n2)
     | _ -> false)
  | Expr_FunctionSymbol (x1, es1) ->
    (match e2 with
     | Expr_FunctionSymbol (x2, es2) -> (&&) (eqb0 x1 x2) (list_beq eqb_Expr es1 es2)
     | _ -> false)
  | Expr_True -> (match e2 with
                  | Expr_True -> true
                  | _ -> false)
...
| Expr_FunctionSymbol (_, es) -> fold_left (fun b e0 -> (&&) b (is_value e0)) es true
| Expr_True -> true
| Expr_False -> true
| _ -> false
...
let update_variable_context c x t =
  ...
  build_context ((fun y -> if eqb0 x y then Some t else fst vc y), (x :: (snd vc))) tc
    ac fc
...
let update_type_context c x n =
  ...
  let ac = get_action_context c in
  let fc = get_function_variable_context c in
  build_context vc ((fun y -> if eqb0 x y then Some n else fst tc y), (x :: (snd tc)))
    ac fc
...
let update_action_context c x params ret_type body =
  ...
  build_context vc tc ((fun y ->
    if eqb0 x y then Some ((params, ret_type), body) else fst ac y), (x :: (snd ac))) fc
...
let rec subst e v x =
  ...
  | Expr_FunctionSymbol (x0, es) ->
    Expr_FunctionSymbol (x0, (map (fun e0 -> subst e0 v x0) es))
...
let rec subst_com p v x =
  ...
  | Com_AssignFun (f, es, e) ->
    Com_AssignFun (f, (map (fun e0 -> subst e0 v x) es), (subst e v x))
  | Com_Seq (p1, p2) -> Com_Seq ((subst_com p1 v x), (subst_com p2 v x))
  | Com_If (e, p0) -> Com_If ((subst e v x), (subst_com p0 v x))
  | Com_IfElse (e, p1, p2) ->
    Com_IfElse ((subst e v x), (subst_com p1 v x), (subst_com p2 v x))
  | Com_For (y, t, p0) -> Com_For (y, t, (subst_com p0 v x))
     ...
     | Some i ->
       (match i with
        | Ivytype_Function (ts, ret_t) ->
          ...
          if list_beq eqb_OptionIvytype (map (fun x0 -> Some x0) ts)
...
let rec check_command_helper p gamma =
  ...
  | Com_Assign (x, e) ->
    ...
    if eqb_OptionIvytype e_type t then Some gamma else None
  ...
  | Com_For (x, t, p') ->
    (match t with
     | Ivytype_Bool ->
       check_command_helper p' (update_variable_context gamma x Ivytype_Bool)
     | Ivytype_UserDefined (y, _) ->
       (match lookup_type gamma y with
        | Some _ -> check_command_helper p' (update_variable_context gamma x t)
        | None -> None)
     | _ -> None)
  | Com_While (e, p') ->
    (match type_expr e gamma with
    ...
    then let arg_ts = map snd arg_names_and_ts in
         (match ret_type with
          | Ivytype_Bool ->
            ...
            if arg_ts_declared
            then Some
                   (update_variable_context gamma var_id (Ivytype_Function (arg_ts,
                     ret_type)))
            else None
          | Ivytype_UserDefined (x, _) ->
            (match lookup_type gamma x with
     ...
     | None ->
       (match ret_type with
        | Ivytype_UserDefined (x, _) ->
          (match lookup_type gamma x with
           | Some _ ->
             ...
             then let gamma' =
                    fold_left (fun acc z ->
                      update_variable_context acc (fst z) (snd z)) arg_ids_and_types
                      gamma
                  in
        ...
        | _ ->
          ...
          if arg_ts_declared
          then let gamma' =
                 fold_left (fun acc z -> update_variable_context acc (fst z) (snd z))
                   arg_ids_and_types gamma
               in
               check_command_helper p'
                 (update_action_context gamma' act_id arg_ids_and_types ret_type p')
          else None))
...
let rec small_step_Expr e gamma s =
  ...
  | Expr_Forall (x, t, e') ->
    (match t with
     | Ivytype_Bool ->
       Some
         (fold_left (fun acc y -> Expr_And (acc, (subst e' y x)))
           (Expr_True :: (Expr_False :: [])) Expr_True)
     | Ivytype_UserDefined (x0, n) ->
       let nums = seq O n in
       let out =
         fold_left (fun acc y -> Expr_And (acc,
           (subst e' (Expr_EnumVarLiteral (t, y)) x0))) nums Expr_True
       in
       Some out
     | _ -> None)
  | Expr_Exists (x, t, e') ->
    (match t with
     | Ivytype_Bool ->
       Some
         (fold_left (fun acc y -> Expr_Or (acc, (subst e' y x)))
           (Expr_True :: (Expr_False :: [])) Expr_False)
     | Ivytype_UserDefined (x0, n) ->
       let nums = seq O n in
       let out =
         fold_left (fun acc y -> Expr_Or (acc,
           (subst e' (Expr_EnumVarLiteral (t, y)) x0))) nums Expr_False
       in
       Some out
     | _ -> None)
  | Expr_Cond (e1, e2, e3) ->
...
let rec small_step_Com p gamma s =
  ...
  | Com_AssignFun (x, args, e) ->
    if is_value e
    then let fcn_sym = Expr_FunctionSymbol (x, args) in
         if is_value fcn_sym
         then Some (Com_Skip, (update_state s fcn_sym e))
         else let args' = small_step_Expr_list args gamma s in
              (match args' with
               | Some _ ->
                 Some ((Com_AssignFun (x, (get_from_maybe_helper args'), fcn_sym)), s)
               | None -> None)
    else (match small_step_Expr e gamma s with
  ...
  | Com_For (x, t, c) ->
    (match t with
     | Ivytype_Bool ->
       Some
         ((fold_left (fun acc y -> Com_Seq (acc, (subst_com c y x)))
            (Expr_True :: (Expr_False :: [])) Com_Skip), s)
        ...
        | Some m' ->
          ...
          let out =
            fold_left (fun acc y -> Com_Seq (acc,
              (subst_com c (Expr_EnumVarLiteral (t, y)) x))) nums Com_Skip
          in
          Some (out, s)
        | None -> None)
     | _ -> None)
  | Com_While (e, c) ->
    Some ((Com_IfElse (e, (Com_Seq (c, (Com_While (e, c)))), Com_Skip)), s)
  | _ -> Some (Com_Skip, s)
